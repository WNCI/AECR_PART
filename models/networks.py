...
from models.AECR_model import AECR_model
from models import utils as util
...
def define_G(input_nc, output_nc, ngf, which_model_netG, opt, mask_global, norm='batch', use_dropout=False,
    ...
    elif which_model_netG == 'unet_AECR':
        netG = UnetGeneratorAECR(input_nc, output_nc, 8, opt, mask_global, AECR_model, cosis_list, cosis_list2, ngf,
                                 norm_layer=norm_layer, use_dropout=use_dropout)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % which_model_netG)

    return init_net(netG, init_type, init_gain, gpu_ids), cosis_list, cosis_list2, AECR_model
...
class UnetGeneratorAECR(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, opt, mask_global, AECR_model, cosis_list, cosis_list2, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(UnetGeneratorAECR, self).__init__()
        ...
        unet_block = UnetSkipConnectionBlock_3(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block,
                                               norm_layer=norm_layer, use_dropout=use_dropout)
        unet_AECR = AECR(ngf * 4, ngf * 8, opt, AECR_model, cosis_list, cosis_list2, mask_global, input_nc=None,
                         submodule=unet_block, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBlock_3(ngf * 2, ngf * 4, input_nc=None, submodule=unet_AECR,
...
class AECR(nn.Module):
    def __init__(self, outer_nc, inner_nc, opt, aecr_model, cosis_list, cosis_list2, mask_global, input_nc, \
        ...
        aecr = AECR_model(opt.threshold, opt.fixed_mask, opt.shift_sz, opt.stride, opt.mask_thred, opt.triple_weight)
        aecr.set_mask(mask_global, 3, opt.threshold)
        aecr_model.append(aecr)
        conn1 = Conn1(strength=opt.strength, skip=opt.skip)
        conn1.set_mask(mask_global, opt)
        cosis_list.append(conn1)

        conn2 = Conn2(strength=opt.strength, skip=opt.skip)
        conn2.set_mask(mask_global, opt)
        cosis_list2.append(conn2)
        ...
        if outermost:
            ...
            upconv_3 = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                          kernel_size=3, stride=1,
                                          padding=1)
            down = [downrelu, downconv, downnorm, downrelu_3, downconv_3, aecr, conn1, downnorm_3]
            up = [conn2, uprelu_3, upconv_3, upnorm_3, uprelu, upconv, upnorm]
